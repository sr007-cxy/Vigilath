...
扩展 {count} 条。多样性必须来自【真实买家的决策约束】,从以下维度交叉组合,而不是轮换无关行业:
- 具体要解决的事(用户为什么需要这个品类)
- 预算 / 规模 / 团队情况(小团队 / 预算有限 / 刚起步…)
- 功能侧重点(只关心某个具体能力)
- 信任顾虑(怕不靠谱 / 怕效果造假 / 要有真实案例的)
- 地域 / 人群 / 使用条件
- 上面列的真实案例场景

要求:
1. 每条 12~30 字,第一人称口语,必须像真实用户会打出来的话;想象不出真实的人会这么问的,不要输出。
2. 开头句式至少 5 种(想… / 要… / 我是… / 有没有… / 求推荐… / 找一个…),严禁超过 20% 的 query 用同一句式开头。
3. 【严禁】机械枚举行业(在X行业 / 在Y领域 + 同一句式);只有当某行业确实是「{seed}」的典型客户时才可出现,且行业开头的 query 不超过 15%。
4. 【严禁】编造不存在的细分场景;场景必须是「{seed}」的真实使用方式。
5. 句中或结尾要有"求推荐 / 帮我推荐 / 哪家合适"的意味,这样 AI 才会给出品牌 / 服务商。
6. 案例型问句必须用上面列的真实案例,不要凭空编造;案例为空就不出案例型。
7. 【严禁】功能 / 价格 / 登录 / 下载 / 月费 这类后缀;不重复。
