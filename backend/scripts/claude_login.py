#!/usr/bin/env python3
"""Claude session management — saves full storage_state (cookies + localStorage).

Mode 1 (local, has GUI):
    python backend/scripts/claude_login.py
    → launches headed browser, manual login via Anthropic account, auto-saves full state

Mode 2 (server, no GUI — import full state JSON):
    python backend/scripts/claude_login.py --import state.json
    → imports a previously exported storage_state file

Mode 3 (export from browser DevTools):
    1. Open claude.ai/new, log in with Anthropic account
...
def show_status():
    ...
    auth_indicators = {"sessionKey", "claude-session", "session",
                       "__Secure-next-auth.session-token",
                       "cf_clearance", "intercom-id"}
...
async def interactive_login():
    ...
    print("Log in with your Anthropic account, then press Enter here to save session.\n")
