#!/usr/bin/env python3
"""Copilot session management — saves full storage_state (cookies + localStorage).

Copilot works WITHOUT login (anonymous mode). A session is only needed for
full features (longer conversations, personality settings, Microsoft account perks).

Mode 1 (local, has GUI):
    python backend/scripts/copilot_login.py
    → launches headed browser, manual login via Microsoft account, auto-saves full state

Mode 2 (server, no GUI — import full state JSON):
    python backend/scripts/copilot_login.py --import state.json
    → imports a previously exported storage_state file

Mode 3 (export from browser DevTools):
    1. Open copilot.microsoft.com, log in with Microsoft account
...
def show_status():
    ...
    if not state:
        print(f"{ENGINE} session: no saved state (anonymous mode — this is OK)")
        return
    ...
    auth_indicators = {"MUID", "_U", "SRCHD", "SRCHUID", "ANON",
                       "WLSSC", "RPSAuth", "MSNRPSSecAuth"}
    found = all_names & auth_indicators
    print(f"  Auth indicators: {', '.join(found) if found else 'none (anonymous mode)'}")
...
async def interactive_login():
    ...
    print("Note: Copilot works without login. Log in only for full features.")
    print("Log in with your Microsoft account if desired, then press Enter.\n")
    ...
    try:
        ...
        print("Page loaded. Log in if you want, or just press Enter for anonymous.")
    except Exception as e:
        print(f"Warning: page load issue ({e}), browser should still be open.")

    await asyncio.get_event_loop().run_in_executor(
        None, input, "\n>>> Press Enter after login (or to save anonymous session): "
    )
