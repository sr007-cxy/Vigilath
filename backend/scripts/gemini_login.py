#!/usr/bin/env python3
"""Gemini session management — saves full storage_state (cookies + localStorage).

Mode 1 (local, has GUI):
    python backend/scripts/gemini_login.py
    → launches headed browser, manual login via Google account, auto-saves full state

Mode 2 (server, no GUI — import full state JSON):
    python backend/scripts/gemini_login.py --import state.json
    → imports a previously exported storage_state file

Mode 3 (export from browser DevTools):
    1. Open gemini.google.com/app, log in with Google account
...
def show_status():
    ...
    auth_indicators = {"SID", "HSID", "SSID", "APISID", "SAPISID",
                       "__Secure-1PSID", "__Secure-1PSIDTS", "NID"}
...
async def interactive_login():
    ...
    print("Log in with your Google account, then press Enter here to save session.\n")
    ...
    try:
        ...
        print("Tip: if Google asks for phone verification, your IP may look risky —")
        print("     try again on a residential network.")
    except Exception as e:
    ...
    print(f"  Google auth cookies:  {', '.join(sorted(found)) if found else 'NONE  ← login did NOT complete'}")

    if not found:
        print("\nERROR: Google login did not complete. Do NOT upload.")
        await browser.close()
        await pw.stop()
        sys.exit(1)
