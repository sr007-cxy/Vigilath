#!/usr/bin/env python3
"""Qwen (通义千问) session management — saves full storage_state.
...
Mode 3 (export from browser DevTools):
    1. Open chat.qwen.ai, log in
    2. DevTools → Console:
       JSON.stringify({
         cookies: document.cookie.split('; ').map(c => {
           const [name,...v] = c.split('=');
           return {name, value:v.join('='), domain:'.qwen.ai', path:'/'};
         }),
...
def show_status():
    ...
    if not state:
        print("Qwen session: no saved state")
        return

    cookies = state.get("cookies", [])
    origins = state.get("origins", [])
    print(f"Qwen session:")
...
async def interactive_login():
    ...
    print("Launching browser for Qwen (通义千问) login...")
...
def main():
    parser = argparse.ArgumentParser(description="Qwen (通义千问) session management")
