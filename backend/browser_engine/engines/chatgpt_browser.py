...
class ChatGPTBrowserAdapter(EngineAdapter):
    ...
    async def is_available(self) -> bool:
        try:
            page, ctx = await create_stealth_page(
                "chatgpt", locale="en-US", timezone_id="America/New_York"
            )
    ...
    async def search(self, query: str) -> EngineResult:
        try:
            # Pin a single device profile for the whole query lifecycle so
            # Cloudflare's UA/platform/WebGL fingerprinting stays consistent.
