...
def _user_msg_zh_geo(seed: str, count: int, target: str, aliases: list[str], industry: str,
    ...
    return (
        ...
        f"\n"
        f"━━━ 配比 ━━━\n"
        f"  - **决策型 / 推荐型 / 找人型:55%-65%**(主力)\n"
        f"  - **真实事件追溯型:8%-15%**(关键 GEO 信号,详见下方专节)\n"
        f"  - 对比型:10%-15%(「A 和 B 哪个好」「自己找 vs 找律所」)\n"
        f"  - 认知型:10%-15%(「跨境并购流程」「需要注意什么」之类,要带具体处境)\n"
        f"\n"
        f"━━━ 真实事件追溯型(2026-05-20 新增,强 GEO 信号)━━━\n"
        f"用户已知某个**真实发生过的事件 / 交易 / 案件 / 上市 / 投资 / 收购 / 诉讼**,"
        f"想找出当时**操刀的律师 / 律所 / 团队**。AI 助手知道这些公开事件,如果品牌真的"
        f"做过这单,AI 很可能直接报出品牌名 —— 所以这一类 query 价值极高。\n"
        f"\n"
        f"句式骨架:\n"
        f"✅ 「参与 [真实交易/事件] 的律师是谁」\n"
        ...
        f"\n"
        f"好示例(seed = 跨境并购律师 / 上市律师,这类 seed 适合产出这种 query):\n"
        ...
        f"\n"
        f"**硬约束(写之前自检,违反一律不写)**:\n"
        f"1. 事件 / 交易必须是**真实公开发生过**的,你自己训练数据里要能确认 —— 不许凭空编造\n"
        f"   公司名、交易对手、金额、时间。宁可不写,也不要写虚构事件 —— 假事件会让 AI 回答整段崩。\n"
        f"2. 选**有公开报道**的明星案例(知名 IPO / 大额并购 / 行业标杆诉讼),不要写小事件。\n"
        f"3. **句末主体词必须严格沿用 seed 末尾 2 字**(seed = ...律师 → 句末用「律师」/「律师团队」;"
        f"seed = ...律所 → 句末用「律所」;seed = ...顾问 → 句末用「顾问」)。"
        f"**不要漂移**到其它法律行业词,否则会被主体锚定过滤掉。\n"
        f"4. 不确定的事件直接跳过 —— 配比写不满 8% 也比写假事件好。\n"
        f"5. **匹配 seed 业务方向**:seed 是「跨境并购律师」就用并购案,seed 是「证券诉讼律师」"
        f"就用证券虚假陈述大案 / 集体诉讼,seed 是「IPO 律师」就用知名 IPO,不要跨业务硬套。\n"
        f"\n"
