...
const ZH: Dict = {
  ...
  hintMetricSource: '问题级覆盖率 — 至少有 1 个模型答复里提到过品牌的监测问题占比。\n· 分子:被任意 1 个模型命中过的不重复监测问题数\n· 分母:监测问题总数\n这里不再乘模型数,所以通常比可见占比高',
...
const EN: Dict = {
  ...
  hintMetricSource: 'Query-level coverage — share of queries that got hit by at least one model.\n· Numerator: distinct queries hit by any model\n· Denominator: total tracked queries\nNot multiplied by #models, so usually higher than Visibility',
