...
const ZH: Dict = {
  ...
  posTail: '末尾',
  posUnknown: '未知',
  hintTopMetrics: '本期 AI 答复里「提到品牌」的回答数(response 维度,基于 hit 判定)。一条答复多次提及只算 1。跟「引用总数」(URL 维度)不同',
...
const EN: Dict = {
  ...
  posTail: 'tail',
  posUnknown: 'unknown',
  hintTopMetrics: 'Number of AI responses this period that mention the brand (response-level, hit-based). Multiple mentions in one response count as 1. Different from "Total Citations" (URL-level)',
