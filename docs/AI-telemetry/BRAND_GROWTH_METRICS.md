...
后端 `GET /topics/{id}/position-breakdown`。**全部按 cell × 全生命周期聚合**,不切 period — 顶栏 period picker 对雷达 5 维不再生效(原始引用、信源等列表页仍按 period 切窗)。

**前置概念**:
- 「cell」= 一对 (监测问题, 模型) 组合;topic 配 N 个问题 + M 个模型 → N×M 个 cell。
- 「cell 命中过」= 该 cell 全生命周期至少有一条答复 hit=True(历史命中也算)。
- 「cell 最佳 brand_rank」= 该 cell 全生命周期所有答复里、品牌被提到位次的最小值(没有则为空)。
- N×M 不随时间变,但 cell 命中状态只增不减 → **雷达 5 维数值单调不减,反映"增长"而非"近期表现"**。重配 topic(增删问题/模型)分母会变,值会跳。

| 维度 | 分子 / 分母 | 作用 |
|---|---|---|
| Top1 占比 | **分子**:cell 最佳 brand_rank = 1 的 cell 数;**分母**:N×M;× 100 | 累计有多少格"曾经做到品牌排第一";数高 = 越来越多的"问题×模型"组合把品牌当首推 |
| Top3 占比 | **分子**:cell 最佳 brand_rank ≤ 3 的 cell 数;**分母**:N×M;× 100 | 累计有多少格曾经进过前三梯队 |
| Top5 占比 | **分子**:cell 最佳 brand_rank ≤ 5 的 cell 数;**分母**:N×M;× 100 | 累计有多少格曾经进过前五名候选 |
| 可见占比 | **分子**:cell 至少被命中过一次的 cell 数;**分母**:N×M;× 100 | 监测网格里曾被刷到过品牌的占比,是覆盖增长的总盘子 |
| 被引用占比 | **分子**:至少在任一模型被命中过一次的监测问题数;**分母**:N(监测问题数);× 100 | 配置的 N 个问题里,有多少个曾经在 AI 答复中刷到过品牌。**与外链引用 URL 无关**,名字易误读 |

**「监测问题数 N」怎么数**:topic 的监测问题列表里,每项只要文本非空就 +1。
**「模型数 M」怎么数**:topic 的引擎列表里,每个非空 engine ID 就 +1。

**为什么不用 period 切窗**:旧口径按 period 切窗、按 response 条数算,会出现"同一 cell 跑 30 次只命中 1 次"被稀释成低数值的问题;period 滚动也让数会**降**(滑出窗口的命中不算了),不符合"GEO 工作产出累积增长"的产品语义。新口径下,只要某 cell 历史上被命中过一次,雷达上这个格子就永久亮 — 看的是品牌**累计覆盖到的网格面积**,而不是"近 30 天命中率"。

**可见占比 vs 被引用占比**:
- 可见占比 = N×M 网格里,有多少格至少被刷到过品牌(粒度细到模型)
- 被引用占比 = N 个问题里,有多少个问题在任一模型上至少被刷到过品牌(只看问题层覆盖)
- 所以同一 topic 里**可见占比 ≤ 被引用占比 / 不一定**:例 N=5, M=4,被引用占比 60% (3 个 query 命中过) 但每个命中 query 只有 1 个模型亮,可见占比 = 3 / 20 = 15%。两者读法不同:可见看"模型 × 问题"两维覆盖密度,被引用只看"问题"覆盖广度。
...
复用 §1.2 的 `position-breakdown`(lifetime 口径,与雷达完全同源),把 4 个维度(Top1 / 可见 / Top5 / 被引用)**单独成卡**:
- 主数 = 该维度 % 值
- 卡底小字 = `行业 P50 X.XX%`,无行业基准时显示「行业基准样本不足」
- 点卡片跳 `/matrix?layer=<dim>`

由于 lifetime 口径,这 4 个数**只会涨不会跌**(除非重配 topic 增问题/模型导致分母变大,或跑 `relabel_topic_target.py` 后清空重算)。用户看到数字变小要先看 topic 是否被重配过。
